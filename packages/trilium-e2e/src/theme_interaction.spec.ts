...
/**
 * The interaction checklist of the re-theme TDD (§6.3), run under each built-in token theme. Theme CSS
 * can break behaviour rather than looks — a menu clipped by an overflow, a row that stops accepting a
 * drop, a toolbar that overflows its bar — so each test drives an interaction and asserts what the app
 * does, not how it looks.
 *
 * Vellum is here because it sets the tree's type smaller, and FancyTree derives its drop zones from
 * row height: the rows it draws are shorter than Lumen's, and dragging has to land the same way.
 *
...
const THEMES = [
    { name: "lumen", stylesheet: "theme-lumen.css" },
    { name: "vellum", stylesheet: "theme-vellum.css" }
];
...
for (const theme of THEMES) {
    test.describe(theme.name, () => {
        ...
        /** The theme the suite was on, restored afterwards: every other spec runs on whatever it finds. */
        let previousTheme = "next";

        test.beforeEach(async ({ page, context }) => {
            const app = new App(page, context);
            await app.goto();
            previousTheme = await page.evaluate(() => (window as any).glob?.theme ?? "next");
            await app.setOption("theme", theme.name);
            await app.goto();
            await expect(page.locator(`link[href*='${theme.stylesheet}']`)).toBeAttached();

            // Whatever ran before may have left the tree hoisted — `help.spec.ts` presses F1, which
            // hoists the User Guide — and these tests start from the fixture's own notes.
            const unhoist = page.locator(".unhoist-button").first();
            if (await unhoist.isVisible()) {
                await unhoist.click();
            }
            await expect(treeNode(app, SAMPLES)).toBeVisible();
        });

        test.afterEach(async ({ page, context }) => {
            const app = new App(page, context);
            await app.setOption("theme", previousTheme);
        });
        ...
        test("The tree context menu opens inside the viewport at the bottom edge", async ({ page, context }) => {
            // Tall enough to hold the whole menu: the tree's menu runs to some 600 px, and a menu taller
            // than the viewport cannot be placed inside it whatever the theme does.
            const height = 900;
            ...
            // The container is the menu itself, and `show()` moves it into whatever holds the screen, so
            // it is found by its id rather than through a parent.
        ...
        test("The classic text editor toolbar stays inside its bar at a narrow width", async ({ page, context }) => {
            ...
            // The fixture edits with the floating toolbar, which has nothing to overflow; the checklist
            // item is the classic one, which the desktop layout draws in a bar of its own below the note.
